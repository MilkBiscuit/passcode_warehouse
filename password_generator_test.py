...
class MyTestCase(unittest.TestCase):
    def test_given_specific_rules_then_generate_expected_password(self):
        for i in range(10):
            pwd = generate_password(lowercase=True, uppercase=False, number=True, custom_chars="",
                                    required_length=8)
            self.assertEqual(8, len(pwd))
            self.assertTrue(pwd.islower())
            self.assertTrue(pwd.isalnum())

            pwd = generate_password(lowercase=True, uppercase=True, number=False, custom_chars="@$", required_length=6)
